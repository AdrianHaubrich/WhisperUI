...
//  WhisperUI
//
//  Created by Adrian Haubrich on 01.06.25.
//
    ...
    var body: some View {
        ZStack {
            if !isComputingWaveform {
                WaveformShape(samples: downsizedSamples, config: config)
    ...
    nonisolated func path(in rect: CGRect) -> Path {
        Path { path in
            ...
            for sample in samples {
                ...
                path.addRect(CGRect(
                    origin: .init(x: x + CGFloat(config.shapeWidth), y: -height / 2),
                    size: .init(width: CGFloat(config.shapeWidth), height: height)
                ))
                
                x += CGFloat(config.spacing + config.shapeWidth)
            }
        }
